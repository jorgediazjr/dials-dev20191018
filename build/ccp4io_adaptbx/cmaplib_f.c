...
static int ioArrayPrint(IOConvMap *ioMap)
{
  ...
  if (rw_mode == 1) {
    ccp4io_fprintf(stdout,"\nFile name for input map file on unit %3d : %s\n",
      ioMap->ipc,filename);
    ccp4io_fprintf(stdout,"file size %ld ; logical name %s\n\n",length,ioMap->logname);
  } else {
    ccp4io_fprintf(stdout,"\nFile name for output map file on unit %3d : %s\n",
      ioMap->ipc,filename);
    ccp4io_fprintf(stdout,"logical name %s\n\n",ioMap->logname);
  }
  out:
...
static int HeaderPrint(const CMMFile *mfile)
{
  ...
  if(!mfile) {
    if (ccp4VerbosityLevel(-1) > 0)
      ccp4io_fprintf(stderr,"WARNING: no header information to print.\n");
    return (0);}

  /* C. Vonrhein: make the output identical to old (pre-5.0) Fortran */
  /*              libraries                                          */
  ccp4io_fprintf(stdout,"\n\n");
  ccp4io_fprintf(stdout,"           Number of columns, rows, sections ...............%5d%5d%5d\n",
          mfile->map_dim[0],mfile->map_dim[1],mfile->map_dim[2]);
  ccp4io_fprintf(stdout,"           Map mode ........................................%5d\n",
	  mfile->data_mode);
  ccp4io_fprintf(stdout,"           Start and stop points on columns, rows, sections %5d%5d%5d%5d%5d%5d\n",
          mfile->origin[0],
  ...
  ccp4io_fprintf(stdout,"           Grid sampling on x, y, z ........................%5d%5d%5d\n",
          mfile->cell_grid[0], mfile->cell_grid[1], mfile->cell_grid[2]);
  ccp4io_fprintf(stdout,"           Cell dimensions .................................%10.4f %10.4f %10.4f %10.4f %10.4f %10.4f\n",
          mfile->cell[0], mfile->cell[1], mfile->cell[2],
          mfile->cell[3], mfile->cell[4], mfile->cell[5]);
  ccp4io_fprintf(stdout,"           Fast, medium, slow axes .........................    %c    %c    %c\n",
          axes[mfile->axes_order[0]],
          axes[mfile->axes_order[1]],
          axes[mfile->axes_order[2]]);
  ccp4io_fprintf(stdout,"           Minimum density .................................%12.5f\n",
          mfile->stats.min);
  ccp4io_fprintf(stdout,"           Maximum density .................................%12.5f\n",
          mfile->stats.max);
  ccp4io_fprintf(stdout,"           Mean density ....................................%12.5f\n",
          (float) mfile->stats.mean);
  ccp4io_fprintf(stdout,"           Rms deviation from mean density .................%12.5f\n",
          (float) mfile->stats.rms);
  ccp4io_fprintf(stdout,"           Space-group .....................................%5d\n",
          mfile->spacegroup);
  ccp4io_fprintf(stdout,"           Number of titles ................................%5d\n",
          ccp4_cmap_number_label(mfile));
  ccp4io_fprintf(stdout,"\n\n");

  ccp4io_fprintf(stdout,"     %-35s\n", "Labels: ");
  {
    int i, nlabels = ccp4_cmap_number_label(mfile);
    for (i=0 ; i != nlabels ; i++)
      ccp4io_fprintf(stdout,"  %s\n",ccp4_cmap_get_label(mfile,i));
  }
  ccp4io_fprintf(stdout,"\n\n");
  return (0);
} 
