...
{
    ccp4io_printf("CCP4 software suite: library version %s\n",ccp4_vers_no());
    ccp4io_printf("CCP4 software suite: patch level     %s\n",ccp4_vers_no());
    ccp4io_printf("Program:             %s",ccp4ProgramName(NULL));
    if (ccp4_prog_vers(NULL) && strlen(ccp4_prog_vers(NULL))) 
      ccp4io_printf("; version %s",ccp4_prog_vers(NULL));
    ccp4io_printf("\n");
}
...
void ccp4ProgramTime(int init)
{
  ...
  if (init || !elaps0 ) {
    ...
    ccp4io_printf("Times: User: %9.1fs System: %6.1fs Elapsed: %5d:%2.2d  \n",
