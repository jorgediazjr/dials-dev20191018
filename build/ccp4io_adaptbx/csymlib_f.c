                ...
                fpstr launam, int launam_len))
{
  ...
  if (!spacegroup || !ccp4spg_pgname_equal(spacegroup->point_group,temp_pgname)) {
    ccp4io_printf("PGNLAU: No spacegroup or incorrect spacegroup loaded! \n");
    free(temp_pgname);
    return;
  }
               ...
               (const int *sindx, int *nlaue, fpstr launam, int launam_len))
{
  ...
  if (*sindx <= 0 || *sindx > MSPAC) {
    ccp4io_printf("Error in CCP4SPG_F_GET_LAUE: sindx %d out of range!\n",*sindx);
    return;
  }

  if ( ! spacegrp[*sindx-1] ) {
    ccp4io_printf("CCP4SPG_F_GET_LAUE: No spacegroup loaded on channel %d ! \n",*sindx);
    return;
  }
                ...
                int pgname_len, fpstr patnam, int patnam_len, int *lpatsg))
{
  ...
  if ( !spacegroup || !ccp4spg_name_equal_to_lib(spacegroup->symbol_xHM,temp_spgnam) ||
    ...
    if ( ! (tmp_spacegroup = ccp4spg_load_by_ccp4_spgname(temp_spgnam)) ) {
      ccp4io_printf("PATSGP: failed to load spacegroup info from SYMINFO! \n");
      free(temp_spgnam);
      free(temp_pgname);
      return;
    }
  ...
  if (!spacegroup ) {
    if (*numsgp > 0) {
      if ( ! (spacegroup = ccp4spg_load_by_ccp4_num(*numsgp)) ) {
        ccp4io_printf("ASUSET: failed to load spacegroup info from SYMINFO! \n");
        ccperror(1,"Fatal error in ASUSET.");
        return;
      }
    } else {
      ccp4io_printf("ASUSET: no spacegroup info! \n");
      ccperror(1,"Fatal error in ASUSET.");
      return;
    }
  }
...
{ 
  ...
  if (*sindx <= 0 || *sindx > MSPAC) {
    ccp4io_printf("Error in CCP4SPG_F_LOAD_BY_NAME: sindx %d out of range!\n",*sindx);
    return;
  }
	       ...
	       (const int *sindx, int *msym, float rrsym[192][4][4]))
{
  ...
  if (*sindx <= 0 || *sindx > MSPAC) {
    ccp4io_printf("Error in CCP4SPG_F_LOAD_BY_OPS: sindx %d out of range!\n",*sindx);
    return;
  }
  ...
  if (!spacegroup ) {
    ccp4io_printf("CCP4SPG_F_LOAD_BY_OPS: no spacegroup info! \n");
               ...
               (const int *sindx, const int ihkl[3], int jhkl[3], int *isym))
{
  ...
  if (*sindx <= 0 || *sindx > MSPAC) {
    ccp4io_printf("Error in CCP4SPG_F_ASUPUT: sindx %d out of range!\n",*sindx);
    return;
  }

  if ( ! spacegrp[*sindx-1] ) {
    ccp4io_printf("CCP4SPG_F_ASUPUT: No spacegroup loaded on channel %d ! \n",*sindx);
    return;
  }
               ...
               (const int ihkl[3], const int *nlaue))
{
  ...
  if (spacegroup->nlaue != *nlaue) {
    ...
    if (ccp4spg_load_laue(spacegroup,*nlaue)) {
      ccp4io_printf("INASU: unrecognised CCP4 Laue code\n");
      return 999;
    }
  }
  ih = ihkl[0];
               ...
               (const int *sindx, const int ihkl[3]))
{
  ...
  if (*sindx <= 0 || *sindx > MSPAC) {
    ccp4io_printf("Error in CCP4SPG_F_INASU: sindx %d out of range!\n",*sindx);
    return 999;
  }

  if ( ! spacegrp[*sindx-1] ) {
    ccp4io_printf("CCP4SPG_F_INASU: No spacegroup loaded on channel %d ! \n",*sindx);
    return 999;
  }
  ih = ihkl[0];
  ik = ihkl[1];
	       ...
	       (const int *sindx, const int ih[3], float *epsi, int *isysab))
{
  ...
  if (*sindx <= 0 || *sindx > MSPAC) {
    ccp4io_printf("Error in CCP4SPG_F_EPSLON: sindx %d out of range!\n",*sindx);
    return;
  }

  if ( ! spacegrp[*sindx-1] ) {
    ccp4io_printf("CCP4SPG_F_EPSLON: No spacegroup loaded on channel %d ! \n",*sindx);
    return;
  }
	       ...
	       (const int *sindx, const int in[3], int *isysab))
{
  ...
  if (*sindx <= 0 || *sindx > MSPAC) {
    ccp4io_printf("Error in CCP4SPG_F_IS_SYSABS: sindx %d out of range!\n",*sindx);
    return;
  }

  if ( ! spacegrp[*sindx-1] ) {
    ccp4io_printf("CCP4SPG_F_IS_SYSABS: No spacegroup loaded on channel %d ! \n",*sindx);
    return;
  }
	       ...
	       (const int *sindx, const int ih[3], int *ic))
{
  ...
  if (*sindx <= 0 || *sindx > MSPAC) {
    ccp4io_printf("Error in CCP4SPG_F_IS_CENTRIC: sindx %d out of range!\n",*sindx);
    return;
  }

  if ( ! spacegrp[*sindx-1] ) {
    ccp4io_printf("CCP4SPG_F_IS_CENTRIC: No spacegroup loaded on channel %d ! \n",*sindx);
    return;
  }
	       ...
	       (const int ih[3], float *cenphs))
{
  ...
  if (! ccp4spg_is_centric(spacegroup, h, k, l) ) {
    ccp4io_printf("CENTPHASE: This is not a centric reflection!\n");
    return;
  }
	       ...
	       (const int *sindx, const int ih[3], float *cenphs))
{
  ...
  if (*sindx <= 0 || *sindx > MSPAC) {
    ccp4io_printf("Error in CCP4SPG_F_IS_CENTPHASE: sindx %d out of range!\n",*sindx);
    return;
  }

  if ( ! spacegrp[*sindx-1] ) {
    ccp4io_printf("CCP4SPG_F_IS_CENTPHASE: No spacegroup loaded on channel %d ! \n",*sindx);
    return;
  }
  ...
  if (! ccp4spg_is_centric(spacegrp[*sindx-1], h, k, l) ) {
    ccp4io_printf("CCP4SPG_F_CENTPHASE: This is not a centric reflection!\n");
    return;
  }
...
{ 
  ...
  if (!spacegroup || spacegroup->spg_ccp4_num != *lspgrp) {
    ...
    if ( ! (tmp_spacegroup = ccp4spg_load_by_ccp4_num(*lspgrp)) ) {
      ccp4io_printf("SETLIM: failed to load spacegroup info from SYMINFO! \n");
      return;
    }
...
{ 
  ...
  if (!spacegroup || spacegroup->spg_ccp4_num != *lspgrp) {
    ...
    /* spacegroup only temporary, as setlim is not expected to
       interact with other calls */
    if ( ! (tmp_spacegroup = ccp4spg_load_by_ccp4_num(*lspgrp)) ) {
      ccp4io_printf("SETLIM_ZERO: failed to load spacegroup info from SYMINFO! \n");
      return;
    }
                ...
                const int *nymin, const int *nzmin, int *nx, int *ny, int *nz))
{
  ...
  if (spacegroup->nlaue != *nlaue) {
    ccp4io_printf("SETGRD: supplied CCP4 Laue code is different from that currently stored\n");
    ccp4io_printf("NLAUE (supplied) = %d\n",*nlaue);
    ccp4io_printf("NLAUE (library)  = %d\n",spacegroup->nlaue);
    ccp4io_printf("(For program FFT and certain spacegroups, this is OK.)\n");
    /* The requested Laue number is different to that for the
       current spacegroup
       Save the current Laue code and load the data for the requested code */
    nlaue_save = spacegroup->nlaue;
    if (ccp4spg_load_laue(spacegroup,*nlaue)) {
      ccp4io_printf("SETGRD: unrecognised CCP4 Laue code, couldn't set FFT grid\n");
      return;
    }
  }
