...
/*   ccp4_general.c
     ...
     A wrapper for vprintf - acts like ccp4io_printf with an additional
...
int ccperror_noexit(int ierr, const char *message)
{
  ...
  if (ierr==0) {
    ...
    if (summary_output(-1)) {
      if (html_log_output(-1)) {
        ccp4io_printf("<!--SUMMARY_END--></FONT></B>\n");
      } else {
        ccp4io_printf("<!--SUMMARY_END-->\n");
      }
    }
...
/* ccp4printf
   ...
   passed to ccp4io_printf, with the remaining arguments being the values
...
int ccp4fyp(int argc, char **argv)
{
  ...
  /* Begin */
  if (diag) ccp4io_printf("CCP4FYP: starting\n");

  if (diag) 
   for (i = 0; i < argc; ++i) 
      ccp4io_printf("ccp4fyp: comand line argument %d %s\n",i,argv[i]);
  ...
  while (iarg < argc && !arg_end) {
    if (diag) ccp4io_printf("CCP4FYP: command line argument %d = \"%s\"\n",iarg,argv[iarg]);

    if (argv[iarg][0] == '-') {
      /* An argument of the form -option */
      if (diag) ccp4io_printf("--> This is an option switch\n");
      ...
      if (diag) ccp4io_printf("--> Truncated and uppercased it is now \"%s\"\n",testarg);
	...
	if (diag) ccp4io_printf("--> Identified -v option:");
	...
	if (strlen(testarg) == 1) {
	  /* -n: Don't read environ.def and default.def */
	  if (diag) ccp4io_printf("--> Identified -n option\n");
	  def_init = 0;
	  env_init = 0;
	...
	if (diag) ccp4io_printf("--> Identified -d option:");
	...
	ccp4printf(1,"Ignoring unrecognised switch \"%s\"\n",argv[iarg]);
	 ...
	 This is the end of the command line switches */
      if (diag) ccp4io_printf("CCP4FYP: end of command line switches\n");
      arg_end = 1;
    }
  }
  ...
  if (ihelp > 0) {
	...
	ihelp = 1;
      ...
      if (diag) ccp4io_printf("Verbose level %d\n",ihelp);
    } else {
      ihelp = 1;
  ...
  if (!cinclude) {
    if (diag) ccp4io_printf("--> CINCL env var has no value assigned\n");
  } else {
    if (diag) ccp4io_printf("--> CINCL is \"%s\"\n",cinclude);
  }

  /* Get value of HOME variable (same notes apply as for CINCL) */
  home = (char *) getenv("HOME");
  if (!home) {
    if (diag) ccp4io_printf("--> HOME env var has no value assigned\n");
  } else {
    if (diag) ccp4io_printf("--> HOME is \"%s\"\n",home);
  }    
  ...
  */
  if (ienviron > 0) {
	...
	return 1;
      ...
      if (diag) ccp4io_printf("CCP4FYP: env file is \"%s\"\n",env_file);
      env_init = 1;
	...
	if (home) {
	  ...
	  if (diag) ccp4io_printf("CCP4FYP: HOME exists, joined filename \"%s\"\n",tmpstr);
	  if (tmpstr) {
	    if (env_file) free(env_file);
	    env_file = tmpstr;
	  } else {
      ...
      if (diag) ccp4io_printf(" environ.def file is \"%s\"\n",env_file);
    } else {
      /* Not enough arguments in the arg list
	 Do clean up and exit */
      if (diag) ccp4io_printf(" no filename found\n");
      ccp4fyp_cleanup(ienv,envname,envtype,envext,logical_name,file_name,
		      file_type,file_ext,env_file,def_file,dir,parser);
  ...
  */
  if (!env_file) {
    ...
    if (diag) ccp4io_printf("--> use standard environ.def file\n");
    std_dir = NULL;
    if (cinclude) {
      std_dir = cinclude;
    } else if (home) {
    ...
    if (env_file) free(env_file);
    if (std_dir) {
      if (diag) ccp4io_printf("--> leading directory is \"%s\"\n",std_dir);
    ...
    if (diag) ccp4io_printf("--> Full path for environ.def is \"%s\"\n",env_file);
  }
  ...
  */
  if (env_init && env_file) {
    if (diag) ccp4io_printf("CCP4FYP: reading environ.def file\n");
    if (diag) ccp4io_printf("--> Full path for environ.def is \"%s\"\n",env_file);
	...
	if (ccp4_parse(line,parser) == 3) {
	  ...
	  if (!env_logical_name || !env_file_type || !env_file_ext) {
		...
		ccp4_utils_malloc(sizeof(char)*(strlen(env_file_ext)+1));
	      ...
	      if (diag) ccp4io_printf("Decoded line: %s = %s.%s\n",envname[ienv],
  ...
  */
  if (idefault > 0) {
	...
	return 1;
      ...
      def_init = 1;
      if (diag) ccp4io_printf("CCP4FYP: def file is \"%s\"\n",def_file);
      /* Check whether this includes the path */
      if (dir) free(dir);
	...
	if (home) {
      ...
      if (diag) ccp4io_printf(" default.def file is \"%s\"\n",def_file);
    } else {
      /* Not enough arguments in the arg list
	 Do clean up and exit */
      if (diag) ccp4io_printf(" no filename found\n");
  ...
  */
  if (!def_file) {
    ...
    if (diag) ccp4io_printf("--> use standard default.def file\n");
    std_dir = NULL;
    if (cinclude) {
      std_dir = cinclude;
    } else if (home) {
    ...
    if (diag) ccp4io_printf("--> Full path for default.def is \"%s\"\n",def_file);
  }
  ...
  */
  if (def_init && def_file) {
    if (diag) ccp4io_printf("CCP4FYP: reading default.def file\n");
    if (diag) ccp4io_printf("--> Full path for default.def is \"%s\"\n",def_file);
	...
	if (diag) ccp4io_printf("Decoded line: %s = %s\n",def_logical_name,def_file_name);
  ...
  */
  ...
  while (iarg < argc) {
    ...
    if (diag) ccp4io_printf("--> Raw logical name: \"%s\"\n",argv[iarg]);
    strtoupper(logical_name,argv[iarg]);
    logical_name[strlen(argv[iarg])] = '\0';
    if (diag) ccp4io_printf("--> Logical name: \"%s\"",logical_name);
    iarg++;
	...
	return ierr;
	 ...
	 Do clean up and exit with error */
      if (diag) ccp4io_printf("  no associated file name\n");
      ccp4fyp_cleanup(ienv,envname,envtype,envext,logical_name,file_name,
		      file_type,file_ext,env_file,def_file,dir,parser);
  ...
  ccp4fyp_cleanup(ienv,envname,envtype,envext,logical_name,file_name,
		  file_type,file_ext,env_file,def_file,dir,parser);
  if (diag) ccp4io_printf("CCP4FYP: ending\n");
  return 0;
}
		...
		char **envtype, char **envext, int *ienv, int no_overwrt)
{
  ...
  /* Begin */
  if (diag) ccp4io_printf("CCP4SETENV: started, ienv = %d\n",*ienv);
  ...
  /* Look for a match between logical_name and the names from
     environ.def */
  if (diag) ccp4io_printf("CCP4SETENV: looking for a match to logical name \"%s\"\n",
		   logical_name);
  ...
  if (icount == *ienv) {
    ...
    if (diag) ccp4io_printf("%s is non-standard logical name\n",logical_name);
    /* Add it to the list
	...
	ccp4_utils_malloc(sizeof(char)*(strlen(file_ext)+1));
      ...
      if (diag) ccp4io_printf("... done extension:\"%s\" ... \n",envext[*ienv]);
      /* Increment the number of values stored */
      ++(*ienv);
      if (diag) ccp4io_printf("CCP4SETENV: now storing %d entries\n",*ienv);
    }
  } else {
    if (diag) ccp4io_printf("CCP4SETENV: Matched logical name for number %d\n",icount);
  }
  ...
  if (diag) ccp4io_printf("CCP4SETENV: supplied file = \"%s\"\n",value);

  /* Get file path */
  if (file_path) free(file_path);
  file_path = ccp4_utils_pathname(value);
  lpath = strlen(file_path)-1;
  if (diag) ccp4io_printf("CCP4SETENV: path = \"%s\"\n",file_path); 

  /* Get file extension */
  if (file_ext) free(file_ext);
  file_ext = ccp4_utils_extension(value);
  lext = strlen(file_ext);
  if (diag) ccp4io_printf("CCP4SETENV: extension = \"%s\"\n",file_ext); 

  /* Get file root */
  if (file_root) free(file_root);
  file_root = ccp4_utils_basename(value);
  lroot = strlen(file_root);
  if (diag) ccp4io_printf("CCP4SETENV: root = \"%s\"\n",file_root);
  ...
  if (!strmatch(value,"/dev/null") && !strmatch(value,"NL:")) {
	...
	if (diag) ccp4io_printf("CCP4SETENV: added extension \"%s\"\n",file_ext);
      } else {
	if (diag) ccp4io_printf("CCP4SETENV: not adding extension for non-standard file name\n");
      }
    }
	...
	*/
	...
	if (cscr) {
	  if (diag) ccp4io_printf("CCP4SETENV: CCP4_SCR = \"%s\"\n",cscr);
	  /* Store in file_path */
	  lpath = strlen(cscr);
	  if (file_path) free(file_path);
	  file_path = (char *) ccp4_utils_malloc(sizeof(char)*(lpath+1));
	  strncpy(file_path,cscr,(lpath+1));
	  if (diag) ccp4io_printf("CCP4SETENV: set file path to CCP4_SCR = \"%s\"\n",file_path);
	} else {
	...
	file_root = tmpstr1;
	lroot = strlen(file_root);
	if (diag) ccp4io_printf("CCP4SETENV: updated file_root = \"%s\"\n",file_root);
	...
	procid = (int) getpid();
	if (diag) ccp4io_printf("CCP4SETENV: initial procid = %d\n",procid);
        procid = procid % CCP4_MODULO;
	if (diag) ccp4io_printf("CCP4SETENV: procid = %d",procid);
	if (file_ext) free(file_ext);
	file_ext = (char*) ccp4_utils_malloc(sizeof(char)*6);
	sprintf(file_ext,"%05d",procid);
        lext = 5;
	if (diag) ccp4io_printf(" giving file extension \"%s\"\n",file_ext);
      }
      /* No special path for this particular extension */
    }
  } else {
    if (diag) ccp4io_printf("CCP4SETENV: detected dev-null\n");
  }
  ...
  if (diag) ccp4io_printf("CCP4SETENV: building filename = \"%s\"\n",file_name);
  lname = lname + lroot;
  file_name = (char *) ccp4_utils_realloc(file_name,sizeof(char)*(lname + 1));
  if (lroot) {
  ...
  if (icount < *ienv) {
    if (strmatch(envtype[icount],"in") && !no_overwrt) {
      /* Does the file exist? */
      if (diag) ccp4io_printf("CCP4SETENV: checking for existence of input file\n");
      if (ccpexists(file_name)) {
	if (diag) ccp4io_printf("CCP4SETENV: \"%s\" can be opened for reading\n",file_name);
      } else {
	/* File doesn't exist/cannot be opened for reading
	   Clean up, set message and return an error */
	if (diag) ccp4io_printf("CCP4SETENV: \"%s\" cannot be opened for reading\n",file_name);
	ccp4io_printf("File: \"%s\"\nCannot be opened for reading\n",file_name);
	ccp4setenv_cleanup(file_ext,file_root,file_path,file_name);
    ...
    if (diag) ccp4io_printf("CCP4SETENV: cannot determine file type (input or output)\n");
  }

  /* Set the environment variable */
  if (ccpputenv(logical_name,file_name)) {
    if (diag) ccp4io_printf("CCP4SETENV: ccpputenv returned okay\n");
  } else {
    /* Unable to set environment variable
       Clean up and exit the program */
    ccp4setenv_cleanup(file_ext,file_root,file_path,file_name);
    ccp4_signal(CCP4_ERRLEVEL(3) | CGEN_ERRNO(CGENERR_CantSetEnvironment),"ccp4setenv",NULL);
    ccp4io_printf("Cannot create environment variable: \"%s\"\n",logical_name);
    return -1;
  }
  ...
  if (diag) {
    ...
    if (tmpstr1) {
      ccp4io_printf("CCP4SETENV: logical name %s has value \"%s\"\n",logical_name,tmpstr1);
    } else {
      ccp4io_printf("CCP4SETENV: failed to fetch value for logical_name \"%s\"\n",logical_name);
    }
  }
...
int ccpputenv(char *logical_name, char *file_name)
{
  ...
  if (logical_name && file_name) {
    ...
    if (tmpstr) {
      ...
      tmpstr[ltmpstr] = '\0';
      if (diag) ccp4io_printf("CCPPUTENV: string going into ccp4_utils_setenv is \"%s\"\n",tmpstr);
...
void ccp4_banner(void) {
  ...
  if (diag) ccp4io_printf("Entering ccp4_banner \n");
  ...
  /* Information line
     ...
     ccp4io_printf(" ### CCP4 %3s: %-17s  %-18s: %-8s##\n",
  ...
  */
  ...
  /* Rest of the banner */
  ccp4io_printf(" ###############################################################\n");
  ccp4io_printf(" User: %s  Run date: %s Run time: %s \n\n\n",
	 ccp4_utils_username(),ccp4_utils_date(date),ccp4_utils_time(time)); 
  ccp4io_printf(" Please reference: Collaborative Computational Project, Number 4. 2011.\n");
  ccp4io_printf(" \"Overview of the CCP4 suite and current developments\". Acta Cryst. D67, 235-242.\n");
  ccp4io_printf(" as well as any specific reference in the program write-up.\n\n");

  if (diag) ccp4io_printf("Leaving ccp4_banner \n");
